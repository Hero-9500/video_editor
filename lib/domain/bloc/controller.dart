...
class VideoEditorController extends ChangeNotifier {
  ...
  Future<File?> exportVideo({
    ...
    final String name = path.basename(videoPath).split('.').first;
    final String outputPath = tempPath + name + ".$format";
    final String pathNew = tempPath + name + name + ".$format";
    videoPath = videoPath.replaceAll(" ", "*");

    print("=================");
    print(videoPath + "======" + name + "======" + outputPath);
    ...
    final String execute =
        " -i '${videoPath.replaceAll("*", " ")}' ${customInstruction ?? ""} $filter ${_getPreset(preset)} $trim -y $outputPath";

    //------------------//
    //PROGRESS CALLBACKS//
    //------------------//
    if (onProgress != null) _config.enableStatisticsCallback(onProgress);
    int code = await _ffmpeg.execute(execute);
    _config.enableStatisticsCallback(null);
    if (isMuted) {
      if (onProgress != null) _config.enableStatisticsCallback(onProgress);
      code = await _ffmpeg.execute("-i '$outputPath' -c copy -an $pathNew");
      _config.enableStatisticsCallback(null);
    }
    ...
    if (code == 0) {
      ...
      return File(isMuted ? pathNew : outputPath);
    } else if (code == 255) {
